...
def generate_batch_prompt(batch: pd.DataFrame) -> str:
    """Generate a single prompt for multiple attributes."""
    
    # Get available columns
    available_columns = set(batch.columns)
    
    # Process all rows to determine their characteristics
    attributes_info = []
    for _, row in batch.iterrows():
        ...
        attr_info = {
            ...
            'source': row.get('sourced_from', '') if 'sourced_from' in available_columns else '',
            'sensitivity': row.get('sensitivity', '') if 'sensitivity' in available_columns else '',
    ...
    prompt = """As a data dictionary expert at Nomura Holdings, provide comprehensive descriptions for the following attributes.
For each attribute, provide a single description that explains both what it represents and its business purpose.
Keep each description under 150 characters.

"""
    
    # Add each attribute's information
    for i, attr in enumerate(attributes_info, 1):
        prompt += f"\nAttribute {i}:"
        prompt += f"\nName: {attr['name']}"
        if attr['source']:
            prompt += f"\nSource: {attr['source']}"
        ...
        prompt += f"\nContext: {', '.join(attr['reasons']) if attr['reasons'] else 'Business-related attribute'}"
        prompt += "\n"
    
    prompt += """
Return the response as a JSON array where each object has the following format:
{
    "attribute_name": "name",
...
[
    {"attribute_name": "attr1", "description": "description1"},
    {"attribute_name": "attr2", "description": "description2"}
]
"""
...
def process_batch(batch: pd.DataFrame, rate_limiter: AdaptiveRateLimiter) -> List[Tuple[str, str]]:
    ...
    try:
        # Generate single prompt for entire batch
        batch_prompt = generate_batch_prompt(batch)
        
        # Get response for entire batch
        response = call_llm_with_retry(batch_prompt, rate_limiter)
        
        # Process batch response with strict validation
        results = []
        original_attrs = set(batch['attribute_name'].tolist())
        processed_attrs = set()
        
        if isinstance(response, list):
            # Create a mapping of attribute names to descriptions
            response_map = {}
            for item in response:
                attr_name = item.get('attribute_name', '')
                description = item.get('description', '')
                
                # Validate attribute name exists in original batch
                if attr_name and attr_name in original_attrs:
                    response_map[attr_name] = description
                    processed_attrs.add(attr_name)
                else:
                    logging.error(f"Received description for unknown attribute: {attr_name}")
            
            # Process each original attribute in order
            for _, row in batch.iterrows():
                attr_name = row['attribute_name']
                if attr_name in response_map:
                    description = response_map[attr_name]
                    results.append((attr_name, description))
                    logging.info(f"Successfully processed attribute: {attr_name}")
                else:
                    results.append((attr_name, 'Failed to generate description'))
                    logging.error(f"Missing description for attribute: {attr_name}")
            
            # Log any discrepancies
            missing_attrs = original_attrs - processed_attrs
            if missing_attrs:
                logging.warning(f"Attributes without descriptions: {missing_attrs}")
                
        else:
            logging.error(f"Unexpected response format: {response}")
            # Fallback: create failed results for all attributes in batch
            results = [(row['attribute_name'], 'Failed to generate description') 
                      for _, row in batch.iterrows()]
        ...
        # Return failed results for all attributes in batch
        return [(row['attribute_name'], 'Failed to generate description') 
                for _, row in batch.iterrows()]
